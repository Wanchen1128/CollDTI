...
def run_dae():
    drug_train = np.loadtxt(r"SNView/SV/feature/drug_vector.txt")
    protein_train = np.loadtxt(r"SNView/SV/feature/protein_vector.txt")

    drug_size=drug_train.shape[1]
    protein_size=protein_train.shape[1]

    print(drug_size,protein_size)
    drug_feature=DAE(drug_train,drug_size,50,16,1,200,[100])
    np.savetxt('SNView/SV/feature/drug_dae_200.txt',drug_feature)

    protein_feature=DAE(protein_train,protein_size,50,32,1,200,[400])
    np.savetxt('SNView/SV/feature/protein_dae_200.txt',protein_feature)
    return drug_feature, protein_feature

def generate_pair():
    interaction = np.loadtxt('data/Ning/mat_drug_protein.txt')
    drug = np.loadtxt('./feature/drug_dae.txt')
    protein = np.loadtxt('./feature/protein_dae.txt')
